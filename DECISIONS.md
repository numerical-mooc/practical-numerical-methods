...
Stage 2 uses an agent as a bounded code-authoring assistant: it may inspect the learner's notebook and add unexecuted cells, but not run code, change existing cells, use external resources, choose the launch, or write the verdict. Two supplied briefs cover a grid search at the accepted reference step, a one-halving time-step check on the five best launches, four outward launches at half the grid spacing to find the edges of the region of launches that tie within the 1 cm target, and an Euler–RK2 comparison for one launch the learner chooses from that region, judged against a candidate reference the learner establishes with the same two-halving rule as Stage 1 (revised 2026-09-06). The lesson states the tie rule before the search and asks the learner to compare the candidate's time-step requirements with Stage 1, showing that a validated time step belongs to a flight, not to a method. 
