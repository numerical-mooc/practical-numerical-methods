...
Stage 2 uses an agent as a bounded code-authoring assistant: it may inspect the learner's notebook and add unexecuted cells, but not run code, change existing cells, use external resources, choose the launch, or write the verdict. Two supplied briefs cover a grid search at the accepted reference step, a one-halving time-step check on the five best launches, four nearby launches at half the grid spacing to explore sensitivity to launch conditions, and an Euler–RK2 comparison for one competitive tested launch the learner chooses, judged against a candidate reference the learner establishes with the same two-halving rule as Stage 1 (revised 2026-09-06). The lesson treats ranges within 1 cm of the best checked range as tied by engineering judgment, rather than claiming numerical indistinguishability or mapping a continuous region (clarified 2026-09-07). It states this decision rule before the search and asks the learner to compare the candidate's time-step requirements with Stage 1, showing that a validated time step belongs to a flight, not to a method.
